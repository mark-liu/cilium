...
func (rc *reconcileCache) populate(ctx context.Context, clientset client.Clientset) error {
...
// recoverVRF is called from the addVRF path when an existing linux
// VRF device is discovered for a VRF that is being added.
//
// It reconciles the enslaved interfaces of the existing device against the
// VRF's desired interfaces, removing stale slaves and adding missing ones.
//
// We are guaranteed to know all specified interfaces in the VRF are available
// on the system when entering this function, assuming no one has modified the
// device out-of-band.
func (c *controller) recoverVRF(ctx context.Context, vrfLink *netlink.Link, vrf *VRF) error {
	vrfIndex := (*vrfLink).Attrs().Index

	// Enumerate all links to find currently enslaved interfaces.
	links, err := safenetlink.LinkList()
	if err != nil {
		return fmt.Errorf("failed to list links for VRF recovery: %w", err)
	}

	currentSlaves := make(map[string]netlink.Link)
	for _, link := range links {
		if link.Attrs().MasterIndex == vrfIndex {
			currentSlaves[link.Attrs().Name] = link
		}
	}

	desired := make(map[string]struct{}, len(vrf.Interfaces))
	for _, iface := range vrf.Interfaces {
		desired[iface] = struct{}{}
	}

	// Remove interfaces that are enslaved but no longer desired.
	for name, link := range currentSlaves {
		if _, ok := desired[name]; !ok {
			c.logger.Info("Removing stale interface from VRF device", fieldInterface, name, fieldVRF, vrf.Name)
			if err := netlink.LinkSetNoMaster(link); err != nil {
				return fmt.Errorf("failed to remove interface %s from VRF %s: %w", name, vrf.Name, err)
			}
		}
	}

	// Enslave interfaces that are desired but not yet enslaved.
	for _, iface := range vrf.Interfaces {
		if _, ok := currentSlaves[iface]; !ok {
			c.logger.Info("Enslaving missing interface to VRF device", fieldInterface, iface, fieldVRF, vrf.Name)
			...
			if err := netlink.LinkSetMaster(child, *vrfLink); err != nil {
				return fmt.Errorf("failed to enslave interface %s to VRF %s: %w", iface, vrf.Name, err)
			}
		}
	}
...
func (c *controller) addVRF(ctx context.Context, rc *reconcileCache, rtxn statedb.ReadTxn, vrf *VRF) error {
	...
	if err != nil && !errors.As(err, &netlink.LinkNotFoundError{}) {
		// transient netlink error, set status and abandon until next retry
		msg := fmt.Sprintf("failed to look up VRF device for VRF %q: %v", vrf.Name, err)
	...
	if vrfLink != nil {
		if err := c.recoverVRF(ctx, &vrfLink, vrf); err != nil {
			msg := fmt.Sprintf("failed to recover VRF device for VRF %q: %v", vrf.Name, err)
			if err := c.setVRFStatus(ctx, vrf.Name, ciliumv2.VRFConditionInterfaceFailure, false, msg); err != nil {
				c.logger.Warn("Failed to set VRF status", fieldVRF, vrf.Name, logfields.Error, err)
			}
			return fmt.Errorf(msg)
...
func (c *controller) updateVRF(ctx context.Context, rc *reconcileCache, vrf *VRF) error {
	...
	// TODO: make this handle interface changes, node selector changes, and
	// pod selector changes gracefully.

	// We need to recreate the VRF completely. This involves removing the
	// old VRF and recreating the new one. Both operations can fail,
	// but this is okay because:
	// Remove fails -> updated VRF never set to active, next iteration will
	// attempt update again.
	// Remove success, create fails -> old VRF has been removed, but new VRF
	// is not set active, next iteration will attempt the creation of VRF as if
	// its new.
...
// clean is ran initially on controller start.
//
// This function will find any VRF interfaces which do not correspond to the
// current list of VRFs that apply to this node, and remove them.
//
// Additionally, after evaluating all table IDs that should exist across all
// VRFs applying to this node, we regenerate any endpoints which have a table ID
// set (are a member of a VRF) that is not present in the list of desired table
// IDs given the set of CiliumVRFs.
//
// Finally, it also removes any stale VRF status entries on the current CiliumNode
// object.
func (c *controller) clean(ctx context.Context, rc *reconcileCache) {
	// Wait for the CiliumVRF reflector to complete its initial list so we
	// have the full set of desired CRDs before deciding what's orphaned.
	rtxn := c.db.ReadTxn()
	...
	// get the VRFs which should exist on this node.
	rtxn = c.db.ReadTxn()
	localNode, err := c.localNodeStore.Get(ctx)
	if err != nil {
		c.logger.Error("Failed to get local node during restore, skipping", logfields.Error, err)
		return
	}

	vrfsByTable := make(map[int32]*VRF)
	vrfsByName := make(map[string]struct{})
	for vrf := range c.vrfs.All(rtxn) {
		if vrf.MatchesNode(localNode.Labels) {
			vrfsByTable[vrf.Table] = &vrf
			vrfsByName[vrf.Name] = struct{}{}
		}
	}

	// Remove all stale VRF statuses from the node in a single update.
	if err := c.updateNodeVRFStatus(ctx, func(vrfStatuses map[string]ciliumv2.VRFNodeStatus) {
		for name := range vrfStatuses {
			if _, desired := vrfsByName[name]; !desired {
				c.logger.Info("Clearing stale VRF status", fieldVRF, name)
				delete(vrfStatuses, name)
			}
		}
	}); err != nil {
		c.logger.Warn("Failed to clear stale VRF statuses during clean", logfields.Error, err)
	}

	// Find all cilium created VRF devices that no longer correspond to the
	// current set of VRFs and remvoe them.
	links, err := safenetlink.LinkList()
	if err != nil {
		c.logger.Error("Failed to list links during restore, skipping", logfields.Error, err)
		return
	}
	...
	for _, link := range links {
		...
		if _, desired := vrfsByTable[tableID]; !desired {
			c.logger.Info("Removing orphaned VRF device", fieldVRFDevice, name, fieldTable, tableID)
			if err := netlink.LinkDel(link); err != nil {
				c.logger.Warn("Failed to delete orphaned VRF device", fieldVRFDevice, name, logfields.Error, err)
			}
		}
	}

	// Regenerate stale endpoints back to the default VRF.
	//
	// an endpoint is stale if its FIB table ID is not in the set of VRFs that
	// apply to this node OR the endpoint's FIB table ID does match a VRF,
	// but the VRF's selectors no longer matches this pod (updated during
	// downtime).
	for _, ep := range c.endpoints.GetEndpoints() {
		...
		vrf, desired := vrfsByTable[int32(fibID)]
		if !desired || !c.matchesEndpoint(ctx, rc, vrf, ep) {
...
//
// On start the controller will find and remove any stale VRF devices, identified
// against the list of current VRFs, and regenerate any endpoints which are
// scoped to these VRF tables, back to the default VRF of 0 (main FIB table).
//
// With all stale VRFs removed, we now treat all VRFs known to the K8s control
// plane as new.
//
// When processing new VRFs we have the ability to determine if an existing
// VRF device exists for the VRF being created, by checking if the Cilium
// created VRF device's bound table ID matches the VRF's table ID. If this
// is the case, we then adopt this orphaned VRF device and configure it to
// match the desired VRF configuration.
//
// They key invariants to remember here are:
// 1. Each applied VRF's table ID must be unique on a node
// 2. Each applied VRF's ID must be unique on a node
// 3. On agent restart c.active is always empty, and all CiliumVRFs are processed
// as new.
//
// Finally, we begin processing general CiliumVRF and device change events,
// keeping Cilium's VRF control-plane in sync with K8s control-plane.
//
// TODO: our reconcile loop does not detect out-of-band changes to our created
// VRF interfaces, or their children. Right now its assumed an operator will not
// make changes to the underlying devices without deleting the effected VRF
// first, or updating the effected VRF shortly after the device modification.
// A trade off has to be considered here, efficiency vs self-healing, since
// netlink calls are not free.
func (c *controller) run(ctx context.Context, health cell.Health) error {
	...
	if err := rc.populate(ctx, c.clientset); err != nil {
		return fmt.Errorf("failed to populate reconcile cache: %w", err)
	}

	// Clean up orphaned VRF devices and stale endpoint state from a
	// previous agent run before the first reconciliation.
	c.clean(ctx, &rc)
